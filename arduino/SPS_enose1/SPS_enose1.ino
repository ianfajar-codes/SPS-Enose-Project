...
const char* pass     = "98765432";
const char* RUST_IP  = "10.44.47.116";   // GANTI KALAU IP BERUBAH
...
void setup() {
  ...
  Serial.println("📝 Timing (TESTING MODE):");
  Serial.println("   - HOLD: 20s (testing)");
  Serial.println("   - PURGE: 40s (testing)");
...
void startSampling() {
  if (!samplingActive) {
    ...
    Serial.println("🎯 5 Levels | Hold: 20s | Purge: 40s (TESTING)");
    Serial.println("🎯 ========================================\n");
  } else {
...
void changeState(State s) {
  ...
  switch(s) {
    ...
    case HOLD: Serial.println("   ⏱  Duration: 20 seconds (testing)"); Serial.print("   🌀 Fan: "); Serial.print(speeds[currentLevel]); Serial.println(" | Pump: OFF"); Serial.println("   📊 Collecting data..."); break;
    case PURGE:
      Serial.println("   ⏱  Duration: 40 seconds (testing)");
...
void sendSensorData() {
  ...
  // TIMESTAMP: Ambil waktu sejak Arduino booting (dalam milidetik)
  unsigned long timestamp = millis();
  
  // GM-XXX
  float no2 = (gas.measure_NO2()  < 30000) ? gas.measure_NO2()  / 1000.0 : -1.0;
  float eth_gm = (gas.measure_C2H5OH()< 30000) ? gas.measure_C2H5OH()/ 1000.0 : -1.0;
  float voc_gm = (gas.measure_VOC()  < 30000) ? gas.measure_VOC()  / 1000.0 : -1.0;
  float co_gm  = (gas.measure_CO()   < 30000) ? gas.measure_CO()   / 1000.0 : -1.0;


  // MiCS-5524
  float Rs = calculateRs();
  float co_m = (Rs > 0) ? ppmFromRatio(Rs / R0_mics, "CO") : -1.0;
  float eth_m = (Rs > 0) ? ppmFromRatio(Rs / R0_mics, "C2H5OH") : -1.0;
  float voc_m = (Rs > 0) ? ppmFromRatio(Rs / R0_mics, "VOC") : -1.0;


  // State names untuk debug
  String stateNames[] = {"IDLE","PRE-COND","RAMP_UP","HOLD","PURGE","RECOVERY","DONE"};
  
  // Format JSON yang kompatibel dengan Rust backend (DENGAN TIMESTAMP)
  String json = "{";
  json += "\"type\":\"data\",";
  json += "\"timestamp\":" + String(timestamp) + ",";  // ← FIELD BARU: timestamp dalam milidetik
  json += "\"sample\":\"Level_" + String(currentLevel + 1) + "_" + stateNames[currentState] + "\",";
  json += "\"co_m\":" + String(co_m, 2) + ",";
  json += "\"eth_m\":" + String(eth_m, 2) + ",";
  json += "\"voc_m\":" + String(voc_m, 2) + ",";
  json += "\"no2\":" + String(no2, 2) + ",";
  json += "\"eth_gm\":" + String(eth_gm, 2) + ",";
  json += "\"voc_gm\":" + String(voc_gm, 2) + ",";
  json += "\"co_gm\":" + String(co_gm, 2) + ",";
  json += "\"state\":" + String(currentState) + ",";
  json += "\"level\":" + String(currentLevel + 1);
  json += "}";
  ...
  if (samplingActive && currentState == HOLD) {
    ...
    if (millis() - lastPrint > 2000) {
      ...
      Serial.print(" | ETH: "); Serial.print(eth_gm, 2);
      Serial.print(" | VOC: "); Serial.print(voc_gm, 2);
      Serial.print(" | CO: "); Serial.println(co_gm, 2);
      Serial.print("   MiCS   → CO: "); Serial.print(co_m, 2);
      Serial.print(" | ETH: "); Serial.print(eth_m, 2);
      Serial.print(" | VOC: "); Serial.println(voc_m, 2);
      lastPrint = millis();
    }
  }
}
